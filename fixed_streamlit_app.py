...
def exchange_code_for_token(auth_code, state):
    """Exchange authorization code for access token with lenient state verification"""
...
def main():
    ...
    # 🔧 FIXED: Check for authorization code in URL and handle redirect loop
    query_params = st.query_params
    if 'code' in query_params and st.session_state.access_token is None:
        auth_code = query_params.get('code')
        state = query_params.get('state', '')
        
        # 🔧 CRITICAL FIX: Clear query parameters IMMEDIATELY to prevent redirect loop
    ...
    if st.session_state.access_token:
        ...
        with st.expander("ℹ️ Setup Information", expanded=True):
            ...
            2. **App Configuration**: Your app should be configured with:
               - **Redirect URI**: `https://dexcom-2zqv2mhmkwgqqedzkjvfeq.streamlit.app`

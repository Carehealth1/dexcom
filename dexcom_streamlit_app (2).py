...
def get_auth_url():
    """Generate the authorization URL (simplified for debugging)"""
    # Skip state parameter for now to avoid complexity
    params = {
        'client_id': CLIENT_ID,
...
def exchange_code_for_token(auth_code, state):
    ...
    # Verify state parameter to prevent CSRF attacks (lenient for testing)
    if 'oauth_state' in st.session_state and state and state != st.session_state.oauth_state:
        st.warning("⚠️ State parameter mismatch - proceeding anyway for testing")
...
def main():
    ...
    st.sidebar.write(f"**Has Access Token**: {'Yes' if st.session_state.access_token else 'No'}")
    ...
    # Debug reset button
    if st.sidebar.button("🔄 Clear Session (Debug)", type="secondary"):
    ...
    if 'code' in query_params and st.session_state.access_token is None:
        ...
        # Clear query parameters immediately to prevent redirect loop
        st.query_params.clear()
    ...
    if st.session_state.access_token:
        ...
        with st.expander("🔍 Debug: OAuth URL", expanded=False):
            st.code(auth_url)
